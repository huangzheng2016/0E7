...
// 返回值：true表示有PENDING流量，false表示没有
func (fd *FlagDetector) detectPendingFlags() bool {
	// 分批处理所有PENDING流量，每批1000条
	batchSize := 1000
	offset := 0
	totalProcessed := 0
	hasMore := true
	...
	for hasMore {
		...
		err := config.Db.Where("tags LIKE ?", "%PENDING%").Limit(batchSize).Offset(offset).Find(&pcaps).Error
		if err != nil {
		...
		if offset == 0 {
			log.Printf("检测到PENDING流量，开始分批处理，每批 %d 条", batchSize)
		}

		log.Printf("处理第 %d 批，共 %d 条PENDING流量", (offset/batchSize)+1, len(pcaps))
		...
		// 更新偏移量，准备处理下一批
		offset += batchSize
		totalProcessed += len(pcaps)

		// 如果当前批次少于batchSize，说明已经处理完所有数据
		if len(pcaps) < batchSize {
			hasMore = false
		}
	}

	log.Printf("PENDING流量处理完成，共处理 %d 条记录", totalProcessed)

	// 返回true表示有PENDING流量被处理
